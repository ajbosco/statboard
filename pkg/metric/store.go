...
type Store interface {
	GetMetric(name string, since time.Time) ([]Metric, error)
...
func (s *stormStore) WriteMetric(m Metric) error {
	m.ID = fmt.Sprintf("%s-%s", m.Date, m.Name)
	return s.db.Save(&m)
}

// GetMetric returns the metrics since the given date
func (s *stormStore) GetMetric(name string, since time.Time) ([]Metric, error) {
	var metrics []Metric
